...
class MCashbankController extends Controller
{
	...
	public function actionUpdate($id)
	{
		...
		if ($modelHeader->state_id == 3 or $modelHeader->state_id == 4) {
			...
			$this->redirect(array('/mCashbank/view','id'=>$modelHeader->id));
		}
	...
	public function actionDelete($id)
	{
		...
		if ($model->state_id == 3 or $model->state_id == 4) {
			...
			$this->redirect(array('/mCashbank/view','id'=>$model->id));
		}
	...
	public function actionCashbankAutoComplete()
	{
		...
		if (isset($_GET['term'])) {
			$qtxt =
			"SELECT system_ref FROM u_journal WHERE module_id = 2 AND system_ref LIKE :name ORDER BY system_ref LIMIT 20";
